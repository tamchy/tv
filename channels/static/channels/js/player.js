TorrentStream.Player = function(container, conf)
{
    ...
    function embedPlugin(container, useInternalPlaylist, style, bgColor, fontColor, callback)
    {
        ...
        if(_browser.name == "ie") {
            ...
            html += '<object id="plugin" classid="clsid:' + clsid + '">';
            html += '<param name="autoplay" value="no" />';
    ...
    function _play(index, playConf, oncompleted)
    {
        ...
        if(playlistSize > 1 && _authLevel() == 0) {
            onSystemMessage("need_auth_playlist");
            return;
        }
    ...
    this.volume = function(addVolume) {
        ...
        if(content) {
            var newVolume = 0;
            try {
                newVolume = _pluginGetAudioVolume() + addVolume * 2;
            }
            catch(e) {
                return;
            }
    ...
    this.position = function(pos) {
        ...
        if(conf.needAuthToSeek && _authLevel() == 0) {
            onSystemMessage("need_auth_seek");
            return false;
        }
        else {
            if( ! triggers.mediaStarted) {
    ...
    this.plistLoad = function(torrentURL){
        content.playlistLoad(torrentURL, 0, 0, 0);
    }
    
    this.plistLoadPlayer = function(contentID){
        content.playlistLoadPlayer(contentID);
    }

    this.plistPlay = function(){
        content.playlistPlay();
    }

    this.plistStop = function(){
        content.playlistStop(true);
    }

    this.plistCount = function(){
        return content.playlistCount;
    }
